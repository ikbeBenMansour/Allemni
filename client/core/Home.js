...
const useStyles = makeStyles(theme => ({
  ...
  tileBar: {
    backgroundColor: 'rba(0, 0, 0, 0.72)',
    textAlign: 'left'
  },
  enrolledTitle: {
    color:'#efefef',
    ...
    return (<div className={classes.extraTop}>
      ...
      <Card className={`${classes.card} ${classes.enrolledCard}`}>
        ...
        {enrolled.length != 0 ? (<Enrollments enrollments={enrolled}/>)
                             : (<Typography variant="body1" className={classes.noTitle}>Pas de cours.</Typography>)
        }
      </Card>
      )}
      <Card className={classes.card}>
        ...
        {(courses.length != 0 && courses.length != enrolled.length) ? (<Courses courses={courses} common={enrolled}/>) 
                             : (<Typography variant="body1" className={classes.noTitle}>Pas de nouveaux cours.</Typography>)
        }
      </Card>
    </div>
    )
