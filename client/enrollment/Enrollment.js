  ...
  const markComplete = () => {
    ...
    return (
      ...
      <ListItem button onClick={selectDrawer(-1)} className={values.drawer == -1 ? classes.selectedDrawer : classes.unselected}>
            ...
            <ListItemText primary={"Aperçu du cours"} />
      </ListItem>
      </List>
      <Divider />
      ...
      <List>
          <ListItem>
        <ListItemText primary={<div className={classes.progress}><span>{totalComplete}</span> sur <span>{enrollment.lessonStatus.length}</span> terminé</div>} />
          </ListItem>
      </List>
    </Drawer>
              {values.drawer == - 1 && 
              <Card className={classes.card}>
                <CardHeader
                  ...
                  subheader={<div>
                        <Link to={"/user/"+enrollment.course.instructor._id} className={classes.sub}>par {enrollment.course.instructor.name}</Link>
                ...
                <CardHeader
                  ...
                  subheader={<Typography variant="body1" className={classes.subheading}>{enrollment.course.lessons && enrollment.course.lessons.length} Cour</Typography>}
                  action={
             ...
             <Card className={classes.card}>
                <CardHeader
                  ...
                  action={<Button onClick={markComplete} variant={enrollment.lessonStatus[values.drawer].complete? 'contained' : 'outlined'} color="secondary">{enrollment.lessonStatus[values.drawer].complete? "Terminé" : "Marquer comme terminé"}</Button>} />
                  <CardContent> 
                      <Typography variant="body1" className={classes.para}>{enrollment.course.lessons[values.drawer].content}</Typography>
                  </CardContent>
                  <CardActions>
                    <a href={enrollment.course.lessons[values.drawer].resource_url}><Button variant="contained" color="secondary">Lien de ressource</Button></a>
                </CardActions>
                </Card></>)}
        </div>)
}